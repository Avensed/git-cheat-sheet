...
3. Слияние и ребейз: При слиянии или ребейзе (переносе изменений) указатели веток перемещаются, чтобы отражать новые изменения, добавленные в историю проекта.
...
git pull <origin> <branch> # получить и слить изменения с origin
...
git cherry-pick --no-commit <commit> # скопировать изменения из коммита без создания нового коммита
