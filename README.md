...
Для решения проблемы есть команда `git checkout -f <branch>`. При указании флага `-f` осуществится переход на указанную ветку. Если написанный **незакомиченный** код вам не нужен, прописываем команду с флагом `-f`.
...
Для решения проблемы можно на месте создать, например, `<branch-2>`, а вторым действием сделать коммит в новой ветке. (*Переносим незакомиченный код в новосозданную ветку*)
...
Если мы пропишем `git branch <branch-1> <commit>` и ветка `<branch-1>` не существовала, Git создаст новую ветку и перенесет её.
...
Только что сделано слияние ветки `<branch-1>` с `<branch-2>` через метод "fast-forward", но мы передумали сливать и решили все вернуть назад.
...
Если вдруг мы не знаем коммит, на котором до слияния была ветка `<branch-1>`, нужно написать вместо `<commit>` `ORIG_HEAD`. Команда `merge` перед переносом ветки записывает старый коммит в файл директорию `.git/ORIG_HEAD`, поэтому можем прописать `cat .git/ORIG_HEAD` для выяснения хэша коммита перед `merge`.

Теперь есть 2 варианта написания команды `git branch -f <branch-1> <commit>`:

1. `git branch -f <branch-1> d9e7d67`
<br>
2. `git branch -f <branch-1> ORIG_HEAD`

Итак для отката слияния веток нам нужно прописать:
...
Если после отката слияния нам захотелось перенести ветку обратно туда же, куда мы раньше и слили ее, то прописываем `git checkout -B <branch-1> <branch-2>`.

<hr>

### **Пример №5**: Удаление ветки, которая не слита
У нас была ветка <branch-2>, которая только ответвляется от <branch-1>. Через команду `git branch -D <branch-2>` мы её удалили. Как её вернуть обратно?

Если команда выше была написано только что, то Git удалил ее только визуально для нас. В системе ветка и её коммиты еще некоторое время хранятся, и если оказалось, что ветка нам была нужна, то отменяем удаление ветки, - это значит ввод команды `git branch <branch-2> <commit>`. Мы как бы создали заново ветку, которую удалили.

*В <commit> указываем "вершину" ветки (последний коммит ветки).*

```bash
git branch -D <branch-2>       # принудительно удаляем ветку
# Осознали, что ветка нужна
git branch <branch-2> <commit> # пересоздаем ветку с её вершиной
```